...
class Main extends Component {
  ...
  componentDidMount () {
    this.socket.on('data_receive', (val) => {
      const encrypted = val.data
      const decrypted = this.crypt.decrypt(this.state.privateKey, encrypted)
      const verified = this.crypt.verify(
  ...
  handleSend (e) {
    ...
    if (this.state.currentData.length !== 0) {
      const signature = this.crypt.signature(this.state.privateKey, this.state.currentData)
      const encrypted = this.crypt.encrypt(this.state.otherPublicKey, this.state.currentData, signature)
      this.setState({
  ...
  render () {
    const colorScheme = ['primary', 'danger']
    const alignScheme = ['right', 'left']
    return (
      <div>
